...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь +
	speedCalories := meanSpeed(action, duration) * kmhInMsec
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(speedCalories, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
